...
def generate_randoms():
    ...
    return satellites, flying_over_mother_Russia, visibility


def calculate_schedule(satellites, flying_over_mother_Russia, visibility):
    ...
    while t_current < T_modeling:
        ...
        for s in range(len(satellites)):
            ...
            if satellites[s][0] == 0:
                fomR_cut = intersection(flying_over_mother_Russia[s],
                                        [[t_current, min(t_current + T_horizon, T_modeling)]])
